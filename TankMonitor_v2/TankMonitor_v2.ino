/*
 * Smart Water Tank Monitoring System with Optimized Sensor Reading
 *
 * Description:
 ...
 * Date: 11-8-2025
 * Version: 2.0 (Optimized Sensor Reading)
 */
...
public:
  SensorReadingManager() : bufferIndex(0), validReadings(0), lastSuccessfulRead(0), 
  ...
  if (pendingDistanceRequest || pendingDistanceRequest_2) {
    ...
    if (success) {
      ...
      if (pendingDistanceRequest) {
        String json = "{\"distance_cm\":" + String(cm, 1) + 
                     ",\"calibration_cm\":" + String(calibration_mm / 10.0f, 1) + "}";
...
void handleBenchmark() {
  if (server.hasArg("run")) {
    ...
    String html = "<html><body><h2>Sensor Benchmark</h2>";
    html += "<form><label>Iterations: <input type='number' name='iterations' value='20' min='5' max='100'></label><br><br>";
    html += "<input type='hidden' name='run' value='1'><input type='submit' value='Run Benchmark'></form>";
    html += "<p>Results will be shown in serial output.</p>";
    html += "<a href='/'>Back to Dashboard</a></body></html>";
